...
public class CustomMediaGrid extends SquareFrameLayout implements View.OnClickListener, IMediaGrid {
    ...
    public CustomMediaGrid(Context context) {
        super(context);
        init(context);
    }

    public CustomMediaGrid(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    private void init(Context context) {
        LayoutInflater.from(context).inflate(R.layout.media_grid_content_custom, this, true);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            setOutlineProvider(new ViewOutlineProvider() {
                ...
                public void getOutline(View view, Outline outline) {
                    outline.setRoundRect(0, 0, view.getWidth(), view.getHeight(), UIUtils.dp2px(6));
                }
            });
        }
        mThumbnail = (ImageView) findViewById(R.id.media_thumbnail);
        mCheckView = (TextView) findViewById(R.id.check_view);
        mGifTag = (ImageView) findViewById(R.id.gif);
        mVideoDuration = (TextView) findViewById(R.id.video_duration);

        findViewById(R.id.iv_large).setOnClickListener(this);
...
//        mCheckView.setCountable(mPreBindInfo.mCheckViewCountable);
    ...
    public void setCheckedNum(int checkedNum) {
        if (checkedNum == UNCHECKED) {
            mCheckView.setText("");
            mCheckView.setBackgroundColor(Color.parseColor("#22000000"));
            return;
        }
        if (checkedNum < 0) return;
        mCheckView.setText("" + checkedNum);
        mCheckView.setBackgroundColor(Color.parseColor("#88000000"));
    }
